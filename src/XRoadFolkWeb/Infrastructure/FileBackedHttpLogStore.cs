...
public sealed class FileBackedHttpLogStore : IHttpLogStore
{
    ...
    public FileBackedHttpLogStore(IOptions<HttpLogOptions> options, ILogStream? stream, ILogger<FileBackedHttpLogStore> log)
    {
        ...
        _stream = stream;
        _filePath = cfg.FilePath ?? "logs/http-logs.log";
    ...
    public IReadOnlyList<LogEntry> GetAll() => _ring.ToArray();

    internal ChannelReader<LogEntry> GetReader() => _channel.Reader;
    internal string FilePath => _filePath;
    internal long MaxFileBytes => _maxFileBytes;
    internal int MaxRolls => _maxRolls;
}
