...
(function(){
  ...
  async function loadPerson(publicId,sourceEl){ window.lastPid=lastPid=publicId; var mySeq=++loadSeq; var els=getPanelEls(); if(!publicId||!els.body){ showInfo('Select a person to view details.'); return; } var cached=personCache.get(publicId); if(cached&&cached.details&&cached.details.length){ clearPanel(); var builder=(window._renderPairsGroupedForPerson||defaultRenderPairsGrouped); var accCached=builder(cached.details); if(mySeq!==loadSeq) return; if(accCached&&els.body) els.body.appendChild(accCached); setPdXml(cached.raw||'',cached.pretty||''); ensureShownAndFocus(); focusFirstAccordionHeader(); } else { showLoading(true); } try{ var data=await fetchDetails(publicId); if(mySeq!==loadSeq) return; showLoading(false); if(!data||data.ok!==true){ buildErrorWithRetry('Failed to load details.',()=>loadPerson(publicId,sourceEl)); ensureShownAndFocus(); return; } var details=Array.isArray(data.details)?data.details:[]; personCache.set(publicId,{ details:details, raw:data.raw||'', pretty:data.pretty||'', ts:Date.now() }); clearPanel(); var builder2=(window._renderPairsGroupedForPerson||defaultRenderPairsGrouped); var acc=builder2(details); if(acc&&els.body) els.body.appendChild(acc); else showInfo('No details to display.'); setPdXml(data.raw||'', data.pretty||''); ensureShownAndFocus(); focusFirstAccordionHeader(); activatePdDetailsSubTab(); if(sourceEl) markSelectedSummaryCard(sourceEl); try{ var link=new URL(window.location.href); link.searchParams.set('gpivPublicId',publicId); if(history&&history.pushState) history.pushState(null,'',link.toString()); }catch{} }catch(err){ if(mySeq!==loadSeq) return; showLoading(false); if(err&&err.name==='AbortError') return; buildErrorWithRetry('Failed to load details.',()=>loadPerson(publicId,sourceEl)); ensureShownAndFocus(); try{ console.error('GetPerson fetch failed',err); }catch{} } }
  ...
  try{ var qs=new URLSearchParams(window.location.search); var pid=qs.get('gpivPublicId'); if(pid){ if(document.readyState==='loading'){ window.addEventListener('DOMContentLoaded',()=>loadPerson(pid)); } else { loadPerson(pid);} } }catch{}
  ...
  if(document.readyState==='loading'){ document.addEventListener('DOMContentLoaded', prerenderShell);} else { prerenderShell(); }
