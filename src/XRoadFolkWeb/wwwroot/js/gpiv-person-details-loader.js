...
(function(){
  ...
  function defaultRenderPairsGrouped(pairs){
    ...
    const allowed=new Set(['Person','PersonPublicInfo']);
    ...
    const groups={}; (pairs||[]).forEach(p=>{ const k=p.key||'', v=p.value||''; const seg=pickGroupFromKey(k); if(!seg) return; let g=seg; if(/^addresseshistory$/i.test(g)) g='AddressesHistory'; if(/^juridicalparentshistory$/i.test(g)) g='JuridicalParentsHistory'; if(/^ssn(?:s)?$/i.test(g)) g='Ssns'; if(/^civilstatushistory$/i.test(g)) g='CivilStatusHistory'; if(/^civilstatuses$/i.test(g)) g='CivilStatuses'; if(/^noteshistory$/i.test(g)) g='NotesHistory'; if(/^notes$/i.test(g)) g='Notes'; if(/^spouses?$/i.test(g)) g='Spouse'; if(/^spouses?history$/i.test(g)) g='SpouseHistory'; if(!allowed.has(g) && g!=='Person' && g!=='PersonPublicInfo') return; (groups[g]=groups[g]||[]).push({k:k,v:v}); });
    allowed.forEach(g=>{ if(g!=='Person'&&g!=='PersonPublicInfo'&&!Object.prototype.hasOwnProperty.call(groups,g)) groups[g]=[]; });
    const keys=Object.keys(groups).sort((a,b)=>{ const al=a.toLowerCase(), bl=b.toLowerCase(); const order=x=> x==='PersonPublicInfo'?0 : (x==='Person'?1:2); return (order(al)-order(bl))||a.localeCompare(b); });
