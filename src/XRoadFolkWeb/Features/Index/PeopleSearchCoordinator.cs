...
namespace XRoadFolkWeb.Features.Index
{
    public sealed class PeopleSearchCoordinator(PeopleService service, PeopleResponseParser parser)
    {
        ...
        public async Task<(string Xml, string Pretty, List<People.PersonRow> Results)> SearchAsync(string? ssn, string? firstName, string? lastName, DateTimeOffset? dateOfBirth, CancellationToken ct = default)
        {
            ...
            List<People.PersonRow> results = _parser.ParsePeopleList(xml);
